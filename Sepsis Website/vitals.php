  ...
  if(isset($_SESSION['pid'])){
    ...
    if(!$row = $result->fetch_assoc())
    {
      ...
      $pname = $row['patient_name'];
        /*$sql="SELECT patient_vital.temperature ,patient_vital.heart_rate,patient_vital.wbc,patient_vital.respiratory_rate,patient_vital.systolicbp,patient_vital.Bilirubin_direct,patient_vital.Platelets,patient_vital.Magnesium,patient_vital.BUN,patient_vital.Calcium,patient_vital.alteredmentation FROM clinical_database.patient_vital WHERE patient_id = ? AND date_of_entry = (SELECT MAX(patient_vital.date_of_entry) FROM clinical_database.patient_vital)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('i', $pid);
        $stmt->execute();
        $result = $stmt->get_result();
        if($row = $result -> fetch_array(MYSQLI_NUM)){
          $php_array = array($row[1],0,$row[0],$row[4],0,0,$row[3],0,0,0,0,0,0,0,0,$row[8],0,$row[9],0,0,$row[5],0,0,$row[7],0,0,0,0,0,0,$row[2],0,$row[6],0,0,0,0,0,0);
          $prev_data = json_encode($php_array);
          echo "<script>var javascript_array = ". $prev_data . "; </script>";
        }*/
  ?>
<!DOCTYPE html>
